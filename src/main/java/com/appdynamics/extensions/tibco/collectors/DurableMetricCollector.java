...
public class DurableMetricCollector extends AbstractMetricCollector {
    ...
    private List<com.appdynamics.extensions.metrics.Metric> getDurableInfo(DurableInfo durableInfo, Metrics metrics) {
        ...
        for (Metric metric : durableMetrics) {
            ...
            } else if (("IsDurableActive").equalsIgnoreCase(name)) {
                // Added as per TPI-CAP request
                // Modified by Anwar Fazaa
                int isDurableActive = durableInfo.isActive() ? 1 : 0;
                value = BigDecimal.valueOf(isDurableActive);
            } else if (("IsDurableConnected").equalsIgnoreCase(name)){
                // isActive is depreciated, so if this provides the same functionality as active this will be used.
                int isDurableConnected = durableInfo.isConnected() ? 1 : 0;
                value = BigDecimal.valueOf(isDurableConnected);
            }
